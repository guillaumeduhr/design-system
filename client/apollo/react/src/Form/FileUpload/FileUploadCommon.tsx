...
import trashcan from "@material-symbols/svg-400/outlined/delete-fill.svg";
...
const FileUpload = ({
  ...
  const getIcon = (isInError: boolean, isLoading: boolean) => {
    if (isInError) {
      return (
        <Svg
          src={error}
          aria-label="erreur"
          className="af-form__file-title-container-icon-error"
        />
      );
    }
    if (isLoading) {
    ...
    return (
      <Svg
        src={check}
        aria-label="succés"
        className="af-form__file-title-container-icon-success"
      />
    );
  };
  ...
  return (
    <div>
      ...
      {globalError && (
        <ItemMessageComponent id={idError} message={globalError} />
      )}
      {instructions && (
